...
function checkNotifications(sym){
	var elm = sym.$('message_button');
	if (notifications>0){
		elm.css('display', 'block');
	}else{
...
function startBlink(id){
    ...
    if (blink!=null && blink.time!=null && blink.show_progress==null){
		...
		if (blink.show_progress) showProgress();
    }
...
function initStatementPanels(){
	statement1_sym = stage_sym.getSymbol("Statement1Back");
	statement2_sym = stage_sym.getSymbol("Statement2Back");
	scrollPanel1 = new iScroll('Stage_Statement1Back_scroller', { scrollX:false , scrollY:true, hScrollbar:false, vScrollbar:true});
	scrollPanel2 = new iScroll('Stage_Statement2Back_scroller', { scrollX:false , scrollY:true, hScrollbar:false, vScrollbar:true});
}
...
function statement1BackPressed(sym){
    if (stage_sym != null){
		var elm = sym.getSymbolElement().css('display', 'none');
        stage_sym.play('intro');
    }
}

function statement2BackPressed(sym){
    if (stage_sym != null){
        var elm = statement1_sym.getSymbolElement();
        elm.css({opacity:0, display:'block'});
        elm.animate({opacity:1}, 500);
...
function showVideo(sym){
	var elm = sym.$('video_iframe');
	elm.css('display', 'block');
	elm.html('<iframe class="youtube-player" type="text/html" width="429" height="230" src="http://www.youtube.com/embed/FjZE3Kz0XK4" frameborder="0" allowfullscreen></iframe>');
	//<iframe width="560" height="315" src="//www.youtube.com/embed/2z7agB0sAZQ" frameborder="0" allowfullscreen></iframe>
}

function hideVideo(sym){
	var elm = sym.$('video_iframe');
	elm.css('display', 'none');
	elm.html('');
}